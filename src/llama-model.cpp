...
ggml_backend_buffer_type_t llama_model::select_buft(int il, bool offload, ggml_type type_k, const uint32_t n_embd_k_gqa, uint32_t kv_size) const {
    buft_list_t buft_list;

    if (offload) {
        buft_list = *pimpl->dev_layer.at(il).buft_list;
    } else {
        buft_list = pimpl->cpu_buft_list;
    }
